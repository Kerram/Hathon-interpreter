...
getUnaryOpType pos expr argType retType = do
  ...
  case hTypeCouldBeEq expType argType of
    True -> return retType
    _ -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Operand's type mismatch. Expected: \"" .
      shows argType . showString "\", but found: " .
...
getBinaryOpType pos exp1 type1 exp2 type2 retType = do
  ...
  case (hTypeCouldBeEq expType1 type1) && (hTypeCouldBeEq expType2 type2) of
    True -> return retType
    False -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Operands' types mismatch. Expected: \"" .
...
getBoolOpType pos exp1 exp2 =
  getBinaryOpType pos exp1 BoolType exp2 BoolType BoolType
...
getComparisionOpType pos exp1 exp2 =
  getBinaryOpType pos exp1 IntType exp2 IntType BoolType
...
getArithmOpType pos exp1 exp2 =
  getBinaryOpType pos exp1 IntType exp2 IntType IntType
...
getEqualityType pos exp1 exp2 = do
  ...
  case hTypeCouldBeEq expType1 expType2 of
    False -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Cannot check equality between values \
      \of different types: \"" . shows expType1 . showString "\" and \"" .
      shows expType2 . showString "\"") pos
    True -> case (containsFunctionalType expType1) || (containsFunctionalType expType2) of
      True -> liftEither $ Left $ addPosInfoToErr
        (showString "TYPECHECKING ERROR: Cannot check equality on types containing\
        \ functional types: \"" . shows expType1 . showString "\" and \"" .
        shows expType2 . showString "\"") pos
...
checkTypes (PExp pos expr prog) = do
  ...
  case containsFunctionalType expType of
    True -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Cannot print value of type \"" .
      shows expType . showString "\"") pos
...
getAndCheckDefType enableRecursion
                   (DFun pos name (TFun _ argType retType) (arg:argsTail) expr) = do
...
getAndCheckDefType enableRecursion (DFun pos name defType [] expr) = do
  ...
  case hTypeCouldBeEq defHType expType of
    ...
    False -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Definition type (with applied arguments) \"" .
      shows defHType . showString "\" mismatch with its body type \"" .
      shows expType . showString "\"") pos

getAndCheckDefType _ (DFun pos _ _ _ _) = liftEither $ Left $ addPosInfoToErr
  (showString "TYPECHECKING ERROR: Too many arguments in definition") pos
  ...
  case condType of
    BoolType -> do
      ...
      case hTypeCouldBeEq thenType elseType of
        False -> liftEither $ Left $ addPosInfoToErr
          (showString "TYPECHECKING ERROR: Return expressions in if statement are \
          \not of equal types. First one is of type \"" . shows thenType .
          showString "\" and second is of type \"" . shows elseType . showString "\"") pos
        True -> return $ getMostConcreteType [thenType, elseType]
    _ -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Condition in if statement has type \"" .
...
getExpType (ELAppend pos exp1 exp2) = do
  ...
  if not (hTypeCouldBeEq type2 (ListType type1)) then
    liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Append expected types: a and [a], but found \"" .
...
getExpType (EList pos elems) = do
  ...
  case allPairsEqual hTypeCouldBeEq listOfTypes of
    False -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Heterogeneous lists are not allowed, \
      \but found one") pos
...
getExpType (EVar pos (Ident name)) = do
  ...
  case M.lookup (Ident name) env of
    Nothing -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Cannot calculate type of " . shows name .
      showString ", because it is undeclared") pos
...
getTypeAfterApplication HeadFunType args = do
  ...
  case expType of
    EmptyList -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Cannot deduce type, because builtin head \
      \function was applied to an empty list") pos
    ListType hType -> case getTailFromArgs args of
      Nothing -> return hType
      Just argsTail -> getTypeAfterApplication hType argsTail
    _ -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Builtin function head was applied to a \
      \non list type") pos
...
getTypeAfterApplication EmptyFunType args = do
  ...
  case expType of
    ...
    _ -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Builtin function empty was applied to a \
      \non list type") pos
  where
...
getTypeAfterApplication TailFunType args = do
  ...
  case expType of
    EmptyList -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Cannot deduce type, because tail function \
      \was applied to an empty list") pos
    ListType _ -> case getTailFromArgs args of
      Nothing -> return expType
      Just argsTail -> getTypeAfterApplication expType argsTail
    _ -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Builtin function tail was applied to non \
      \list type") pos
...
getTypeAfterApplication (FunType argType retType) args = do
  ...
  case hTypeCouldBeEq argType expType of
    ...
    False -> liftEither $ Left $ addPosInfoToErr
      (showString "TYPECHECKING ERROR: Type mismatch during function application, \
      \expected \"" . shows argType . showString "\", but found \"" .
      shows expType . showString "\"") pos

getTypeAfterApplication hType args = liftEither $ Left $ addPosInfoToErr
  (showString "TYPECHECKING ERROR: Trying to apply value of type \"" .
